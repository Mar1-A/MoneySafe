...
def load_filenames():
	...
	for file in os.listdir('/content/drive/MyDrive/proj3/data/librispeech_processed/train-clean-100'):
		if file not in ['.', '..', '.DS_Store']:
			# Appending each file to the train_files array
			train_files.append(file)

	# Looping through file in the testing directory
	for file in os.listdir('/content/drive/MyDrive/proj3/data/librispeech_processed/test-clean'):
		if file not in ['.', '..', '.DS_Store']:
    ...
    for file in os.listdir('/content/drive/MyDrive/proj3/data/librispeech_processed/train-clean-100'):
        if file not in ['.','..','.DS_Store']:
            if file.endswith('.npy'):
                train_arr = np.load(os.path.join('/content/drive/MyDrive/proj3/data/librispeech_processed/train-clean-100', file), allow_pickle=True);
...
def compute_mfcc2(audio_data, sample_rate, num_mfcc, frame_size, frame_stride, n_fft, n_mels, hop_length, win_length):
    # Pre-emphasis filter
    pre_emphasis = 0.97
