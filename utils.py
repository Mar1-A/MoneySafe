...
def compute_log_linear_spectrogram(
    audio, sample_rate, window_size=10, step_size=10, max_freq=None, eps=1e-10
):

    # Making sure the step size is no bigger than the window size.
    if step_size > window_size:
        raise ValueError(
            "The step size should be less than or equal to the window size!"
        )
    ...
    f, t, spec = signal.spectrogram(
        ...
        fs=sample_rate,
        window="hann",
        nperseg=samples_per_winseg,
        noverlap=samples_per_step,
        detrend=False,
    )
...
def calculate_input_max_len(dataset):
    ...
    if dataset == "librispeech":
        ...
        train_path = os.path.join(
            args.parameter_name.librispeech_path, "train-clean-100"
        )
        test_path = os.path.join(args.parameter_name.librispeech_path, "test-clean")
