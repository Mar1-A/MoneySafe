...
class SpeechRecognitionModel(object):
    def __init__(self, mode, embedding_matrix=None):
        ...
        with tf.compat.v1.variable_scope("LSTM"):
            # Create the forward and backward LSTM cells with dropout
            lstm_fw_cells = [tf.keras.layers.LSTMCell(args.num_hidden, dropout=args.dropout_rate, recurrent_dropout=args.dropout_rate) for _ in range(args.num_layers)]
            lstm_bw_cells = [tf.keras.layers.LSTMCell(args.num_hidden, dropout=args.dropout_rate, recurrent_dropout=args.dropout_rate) for _ in range(args.num_layers)]

            # Stacking rnn cells
            stack_fw = tf.keras.layers.StackedRNNCells(lstm_fw_cells)
            stack_bw = tf.keras.layers.StackedRNNCells(lstm_bw_cells)

            # Use bidirectional_dynamic_rnn instead of dynamic_rnn  keras.layers.RNN(cell))
            (outputs_fw, outputs_bw), _ = tf.compat.v1.nn.bidirectional_dynamic_rnn(stack_fw, stack_bw, self.inputs, seq_lens, dtype=tf.float32)

            # Combine the outputs of the forward and backward cells
            self.outputs = tf.concat([outputs_fw, outputs_bw], axis=-1)
        ...
        if self.mode == model_modes.TRAIN:
            ...
            # Define the L1 and L2 regularization parameters
            l1_reg = 0.001
            l2_reg = 0.001

            # Add L1 and L2 regularization terms to the cost function
            with tf.compat.v1.variable_scope("Regularization"):
                regularizer = regularizers.l1_l2(l1=l1_reg, l2=l2_reg)
                reg_term = regularizer(W)
    ...
    def save(self, path, sess, global_step=None):
        ...
        with open(os.path.join(args.hparams_path), 'wb') as f:
            pickle.dump(args, f)

        # Save the current state of the model
        self.saver.save(sess, path, args.global_step)
